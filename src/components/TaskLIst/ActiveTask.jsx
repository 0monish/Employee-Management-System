...
const ActiveTask = ({ taskData, updateTaskStatus }) => {
    const handleTaskStatus = (id, taskStatus) => {
        if (taskStatus === "completedTask") {  
            updateTaskStatus(id, "completedTask");
        }
        else if (taskStatus === "failedTask") {
            updateTaskStatus(id, "failedTask");
        } else {
    ...
    return (
        <div className="flex-shrink-0 h-full w-[300px] p-5 bg-green-400 hover:scale-105 text-white rounded-xl">
            ...
            <div className="flex justify-around">
                <button
                    className="bg-blue-400 font-medium active:scale-95 hover:scale-105 text-black px-3 py-1 rounded-md mt-5"
                    onClick={() => handleTaskStatus(taskData.id, "completedTask")}
                >
                    Completed
                </button>

                <button
                    className="bg-red-400 font-medium active:scale-95 hover:scale-105 text-black px-3 py-1 rounded-md mt-5"
                    onClick={() => handleTaskStatus(taskData.id, "failedTask")}
                >
                    Failed
                </button>
            </div>
